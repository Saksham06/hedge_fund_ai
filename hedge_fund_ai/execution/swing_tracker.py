...
class SwingTracker:
    ...
    def check_exits(self, current_prices: dict[str, float]) -> dict[str, str]:
        """
        Check every open position for exit conditions.
        Returns {ticker: exit_reason} for positions that should be closed.

        Exit reasons:
          "max_hold"      — held too long (> SWING_MAX_HOLD_DAYS)
          "profit_target" — hit 12% gain
          "stop_loss"     — hit 5% loss
          "min_hold"      — too recent, do NOT exit yet
        """
        ...
        for ticker, pos in self._open.items():
            ...
            # Never exit before minimum hold period
            ...
            elif pnl_pct <= -SWING_STOP_LOSS:
                exits[ticker] = "stop_loss"
                logger.warning("SWING EXIT (stop): %s %.1f%%", ticker, pnl_pct*100)
