...
MAX_STALE_HOURS    = 36     # warn if data older than this

def _effective_max_stale_hours() -> float:
    """
    Adjust stale threshold for weekends.
    Friday 4PM ET → Monday run = ~64h old — this is normal, not stale.
    Threshold: 36h on weekdays, 72h on Monday (to cover weekend).
    """
    from datetime import datetime, timezone
    weekday = datetime.now(timezone.utc).weekday()
    if weekday == 0:   # Monday
        return 72.0    # weekend gap is normal
    return float(MAX_STALE_HOURS)
    ...
    for d in data:
        ...
        if price_date:
            try:
                ...
                if age_h > _effective_max_stale_hours():
